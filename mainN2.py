from fullProcessModule2 import Module
import numpy as np
import json
import time
...
resultDir="LTSpiceSimResults/fullN2_3_5.json"
...
nodes={"BER":BER_nodes,"Pot":Pot_nodes,"Aux":Aux_nodes}
trigger=1

def getClosest(val,lista):
    valList=[float(v) for v in lista]
    diff=99999999
    best=0
    for i in range(len(valList)):
        if abs(val-valList[i])<diff:
            diff=abs(val-valList[i])
            best=lista[i]
    return best

'''obj=Module(VPPMfreq,1000,numPointsPerPeriod,numSamples,0.2,0,0,22,[-1,1e-7])
obj.GenerateInput()
lista=list(simData.keys())
addicionalNoises=GenerateOtherWaves(getClosest(1,lista),getClosest(1,list(simData[list(simData.keys())[0]].keys())),obj)
        ...
        for Y_Distance in simData[list(simData.keys())[0]].keys():
            ...
            for dc in DCs:
                for n in range(numRepetitions):
                    t0=time.time()
                    obj=Module(VPPMfreq,numBits,numPointsPerPeriod,numSamples,dc,X_Distance,Y_Distance,lux,[-1,noiseAmp],numPackets,True)
                    ...
                    if num<numRepetitions:
                        # Full process run
                        gain=min(2**np.ceil(np.log2(1.4/(obj.lux*40*45000*75*(10**(-9))))),128)
                        r1=10000*(gain-1)
                        if r1<100: r1=100
                        print("Gain:",gain,"R1:",r1)
                        obj.GenerateInput()
                        addicionalNoises=GenerateOtherWaves(X_Distance,Y_Distance,obj)
                        simStr,errors,potencies=obj.Run(circuit,nodes,trigger,{"R_PGA":int(r1)},addicionalNoises)
