...
circuit="circuit_full_ideal.asc"
resultDir="LTSpiceSimResults/results2.json"
...
'''obj=Module(VPPMfreq,1000,numPointsPerPeriod,numSamples,0.5,1,1,20,[-1,1e-8])
obj.Run(circuit,nodes,2)
input("Teste")'''
            ...
            for dc in DCs:
                for n in range(numRepetitions):
                    ...
                    if num<numRepetitions:
                        gain=2**np.ceil(np.log2(1.4/(obj.amp*45000)))
                        r1=10000*(gain-1)
                        if r1<10: r1=10
                        # Full process run
                        simStr,errors=obj.Run(circuit,nodes,trigger,{"R_PGA":int(r1)})
                        # Save results
