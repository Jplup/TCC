...
def Run(nodeNames):
    asc_path = Path(__file__).parent / "AmpOps.asc"
...
def VPPMGenerator(dutyCycle):
    ...
    # Save input to .txt
    with open("OpAmpVoltTest.txt", "w") as f:
...
for DC in DCs:
    t0=time.time()
    ts,ys=VPPMGenerator(DC)
    nodeNames=['V(u1)','V(u2)','V(u16)','V(u128)',
            'V(o1)','V(o2)','V(o16)','V(o128)','V(vin)']
