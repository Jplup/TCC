...
def main():
    ...
    with open("paralel4.json") as fs: paralelData=json.load(fs)

    totalNum=len(paralelData["itemData"]) # Número total de simulações
    print("4: Número total de simulações:",totalNum)
    ...
    for item in paralelData["itemData"]:
        ...
        print("4:",percent,"% / Time to finish:",format_time(timeToFinish),"key runnig:",obj.GetDictKey())
        cont+=1
