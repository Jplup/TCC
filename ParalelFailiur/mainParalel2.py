...
with open("paralel2.json") as fs: paralelData=json.load(fs)

totalNum=len(paralelData["itemData"]) # Número total de simulações
print("2: Número total de simulações:",totalNum)
...
for item in paralelData["itemData"]:
    ...
    print("2:",percent,"% / Time to finish:",format_time(timeToFinish),"key runnig:",obj.GetDictKey())
    cont+=1
